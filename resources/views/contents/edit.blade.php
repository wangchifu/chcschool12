...
@section('content')
    <div class="row justify-content-center py-4">
        <div class="col-md-11">
            <h1 class="fw-bold mb-3"><i class="fas fa-edit me-2"></i>修改內容</h1>

            <div class="card shadow-sm border-0">
                <div class="card-header bg-light py-3">
                    <h5 class="mb-0 fw-bold">內容資料</h5>
                </div>
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('contents.update', $content->id) }}" method="POST" id="this_form">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="group_id" id="group_id" class="form-select">
                                @foreach($group_array as $id => $name)
                                    <option value="{{ $id }}" {{ old('group_id', $content->group_id) == $id ? 'selected' : '' }}>
                                        {{ $name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <small class="text-muted ms-2">(請用英文逗號 `,` 分隔多個標籤)</small>
                            <input type="text" name="tags" id="tags" class="form-control" placeholder="標籤" value="{{ old('tags', $content->tags) }}">
                        </div>

                        {{-- 內文 (CKEditor) --}}
                        <div class="mb-3">
                            <label for="my_editor" class="form-label fw-bold">內文 <span class="text-danger">*</span></label>
                            <textarea name="content" id="my_editor" class="form-control" required>{{ old('content', $content->content) }}</textarea>
                        </div>
